...
function openForm(record = null) {
    ...
    <form id="record-form" class="px-4 pb-10">
      ...
      <div class="form-group p-4">
        ...
        <div class="flex flex-wrap gap-2">
          <button type="button" id="btn-camera" class="btn-media">📷 カメラ</button>
          <button type="button" id="btn-gallery" class="btn-media">🖼️ ライブラリ</button>
        </div>
        <input type="file" id="media-camera" accept="image/*,video/*" capture="environment" class="hidden" />
        <input type="file" id="media-gallery" accept="image/*,video/*" multiple class="hidden" />
        <p class="text-[11px] text-gray-400 mt-2">端末の容量がある限り保存できます</p>
      </div>
  ...
  // カメラ（その場撮影）
  $('#btn-camera').addEventListener('click', () => $('#media-camera').click());
  // ライブラリ（既存アップロード）
  ...
  $('#media-camera').addEventListener('change', async (e) => {
