...
async function openDetail(id) {
      ...
      ${mediaItems.length ? `
        ...
        <div class="form-group p-4">
          <div class="grid grid-cols-2 gap-2">
            ${mediaItems.map((m, i) => `
              <button type="button" class="detail-media-thumb" data-idx="${i}">
                ${m.type === 'video'
                  ? `<div class="relative"><video src="${m.url}" class="w-full aspect-square object-cover rounded-xl bg-black" muted playsinline></video><span class="media-play-badge">▶ 動画</span></div>`
                  : `<img src="${m.url}" class="w-full aspect-square object-cover rounded-xl bg-gray-50" alt="" />`}
              </button>
            `).join('')}
          </div>
  ...
  $$('.detail-media-thumb').forEach(btn => {
    btn.addEventListener('click', () => {
      const idx = Number(btn.dataset.idx);
...
function openLightbox(m, record) {
  ...
  overlay.innerHTML = `
    ...
    ${m.type === 'video'
      ? `<video src="${m.url}" controls autoplay playsinline class="lightbox-media"></video>`
