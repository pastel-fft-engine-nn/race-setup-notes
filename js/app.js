...
function renderList() {
  ...
  container.innerHTML = list.map(r => {
    ...
    const hasDayMedia = dayHasMedia(r.date);
          ...
          <div class="p-4">
            <div class="flex items-start justify-between gap-2 mb-2">
              ...
              <div class="flex flex-wrap gap-1 justify-end">
                ...
                ${mediaCount > 0 ? '<span class="badge badge-media">📷</span>' : ''}
              </div>
            </div>
            ...
            <div class="mt-3 flex items-center justify-between gap-2">
              <span class="text-[11px] ${hasDayMedia ? 'text-pink-500 font-semibold' : 'text-gray-300'}">
                ${hasDayMedia ? '📷 この日の写真あり' : 'この日の写真なし'}
              </span>
              ${hasDayMedia ? `<button type="button" class="btn-day-media" data-date="${escapeHtml(r.date)}">写真を確認</button>` : ''}
            </div>
          </div>
        </article>
      </div>
  ...
  container.querySelectorAll('.swipe-content').forEach(card => {
    card.addEventListener('click', (e) => {
      if (e.target.closest('.btn-day-media')) return;
  ...
  // その日の写真確認
  container.querySelectorAll('.btn-day-media').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      openDayMediaViewer(btn.dataset.date);
    });
  });
...
/** その日の写真・動画ビューア */
function openDayMediaViewer(dateStr) {
  const items = getDayMedia(dateStr);
  if (!items.length) {
    toast('この日の写真はありません');
    return;
  }
  ...
  $('#modal-title').textContent = `${formatDate(dateStr)} の写真`;
  $('#modal-body').innerHTML = `
    <div class="px-4 pb-10">
      <p class="text-[13px] text-gray-400 mb-3">${mediaWithUrl.length} 件の写真・動画</p>
      <div class="grid grid-cols-1 gap-3">
        ${mediaWithUrl.map(m => m.type === 'video'
          ? `<video src="${m.url}" controls class="w-full rounded-xl bg-black" playsinline></video>`
          : `<img src="${m.url}" class="w-full rounded-xl object-contain bg-gray-50" alt="" />`
        ).join('')}
      </div>
    </div>
  `;
